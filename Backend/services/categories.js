...
export default class CategoryService extends Service {
  ...
  async get(filter) {
    return this.repository
      .find(filter)
      .populate({ path: "games", select: "gameName" });
  }
  ...
  async create(category, games) {
    ...
    await this.repository.findOneAndUpdate(category, add, { upsert: true });

    const result = !!Object.values(add).length;

    if (result === false) {
      throw new Error("Categoria Criada");
    } else {
      let cat = await this.repository.findOne(category);
  ...
  async foundCat(_id) {
    let teste = await this.repository.findById(_id);
    return teste;
  }
}
