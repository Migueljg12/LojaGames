...
router.post("/insert/:_id", auth, async (req, res) => {
  const { params } = req;
  const { body } = req;
  try {
...
router.get("/search", auth, async (req, res) => {
  ...
  try {
    ...
    res.status(200).json("Mandando essa categoria >>>", categories);
  } catch ({ message }) {
    res.status(400).json({ message });
  }
});
...
router.get("/search/:_id", auth, async (req, res) => {
  ...
  console.log(params);
  try {
    const categories = await service.foundCat(params);
    console.log("retornanado categoria");
